...
export async function setOrderFile(userId, orderId, { storagePath, originalFilename }) {
  ...
  const res = await pool.query(
    `UPDATE orders SET
      ...
      status = 'done',
      is_done = true
     WHERE ${isNaN ? "order_code" : "id"} = $4 AND user_id = $5
     RETURNING *`,
    [storagePath, originalFilename, now.toISOString(), isNaN ? orderId : parsedId, Number(userId)]
  );
  if (res.rowCount === 0) return null;
  return mapOrder(res.rows[0]);
}
...
export async function setOrderExternalLink(userId, orderId, externalLink) {
  ...
  const res = await pool.query(
    `UPDATE orders SET
      ...
      status = 'done',
      is_done = true
     WHERE ${isNaN ? "order_code" : "id"} = $3 AND user_id = $4
     RETURNING *`,
    [externalLink, now.toISOString(), isNaN ? orderId : parsedId, Number(userId)]
  );
  if (res.rowCount === 0) return null;
  return mapOrder(res.rows[0]);
}
...
export async function recordReminderSent(orderId) {
  ...
  await pool.query(
    `UPDATE orders SET
      ...
      last_reminder_at = $1
     WHERE ${isNaN ? "order_code" : "id"} = $2`,
    [now.toISOString(), isNaN ? orderId : parsedId]
  );
}
...
export async function clearOrderFile(orderId) {
  ...
  await pool.query(
    `UPDATE orders SET
      ...
      file_downloaded_at = NULL,
      file_delete_at = NULL
     WHERE ${isNaN ? "order_code" : "id"} = $1`,
    [isNaN ? orderId : parsedId]
  );
}
...
export async function markFileDownloaded(orderId) {
  ...
  await pool.query(
    `UPDATE orders SET
      ...
      file_delete_at = $2
     WHERE ${isNaN ? "order_code" : "id"} = $3 AND file_downloaded_at IS NULL`,
    [now.toISOString(), deleteAt.toISOString(), isNaN ? orderId : parsedId]
  );
}
...
export async function confirmPayment(userId, orderId) {
  ...
  const res = await pool.query(
    `UPDATE orders SET is_paid = true
     WHERE ${isNaN ? "order_code" : "id"} = $1 AND user_id = $2
     RETURNING *`,
    [isNaN ? orderId : parsedId, Number(userId)]
  );
  if (res.rowCount === 0) return null;
  return mapOrder(res.rows[0]);
}
...
export async function createNotification(userId, orderId, type, message) {
  ...
  if (orderId) {
    ...
    if (!Number.isNaN(parsedId)) {
      resolvedOrderId = parsedId;
    } else {
      const res = await pool.query("SELECT id FROM orders WHERE order_code = $1 LIMIT 1", [orderId]);
      resolvedOrderId = res.rows[0]?.id || null;
    }
  }
  const res = await pool.query(
    `INSERT INTO notifications (user_id, order_id, type, message)
     VALUES ($1, $2, $3, $4)
     RETURNING id`,
    [userId ? Number(userId) : null, resolvedOrderId, type, message]
  );
  return res.rows[0];
}
...
export async function isUsernameAvailable(username, excludeUserId) {
  ...
  const res = await pool.query(
    ...
    [username.toLowerCase(), Number(excludeUserId)]
  );
  return res.rows.length === 0;
}

export async function updateUserProfile(userId, { username, name }) {
  await initDb();
  const res = await pool.query(
    `UPDATE users SET username = $1, name = $2, updated_at = NOW() WHERE id = $3 RETURNING id, username, name`,
    [username.toLowerCase(), name, Number(userId)]
  );
  return res.rows[0];
}
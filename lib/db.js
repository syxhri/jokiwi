 ...
 * Passwords are stored as bcrypt hashes. Returns the
 * created user.
 *
...
export async function verifyUserPassword(user, password) {
  ...
  // Backward compatibility: old db stored plain password in `password`.
  if (user.password && String(user.password) === raw) {
    // Opportunistically migrate to passwordHash.
    try {
      const db = await getDb();
      await db.read();
      const idx = db.data.users.findIndex(u => u.id === user.id);
      if (idx !== -1) {
        db.data.users[idx].passwordHash = await bcrypt.hash(raw, 10);
        delete db.data.users[idx].password;
        await db.write();
      }
    } catch {
...
export async function setUserQrisPayload(userId, qrisPayload) {
  ...
  u.qrisPayload = qrisPayload ? String(qrisPayload).trim() : null;
  await db.write();
  return u;
}

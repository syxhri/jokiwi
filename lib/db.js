...
export async function getAllOrdersForUser(userId) {
  ...
  userId = (await findUser(userId))?.id;
  const res = await pool.query(
    "SELECT id, user_id, order_code, client_name, task_name, category_id, category_code, category_name, price, is_done, is_paid, notes, created_at, assigned_date, deadline_date FROM orders WHERE user_id = $1",
    [Number(userId)]
  );

...
export async function updateCategory(userId, id, patch) {
  ...
  const updated = {
    name: (patch.name !== undefined && patch.name !== null && (patch.name || "").trim() !== "") ? String(patch.name).trim() : current.name,
    description:
      (patch.description !== undefined && patch.description !== null && (patch.description || "").trim() !== "") ? patch.description : current.description,
    notes: (patch.notes !== undefined && patch.notes !== null && (patch.notes || "").trim() !== "") ? patch.notes : current.notes,
  };
  await pool.query(
...
export async function getAllOrdersForUser(userId) {
  ...
  const orders = res.rows.map((row) => ({
    ...
    categoryCode: row.category_code || null, // ← dari tabel categories
  ...
  // tetap urutkan by assigned_date desc
...
export async function updateOrder(userId, id, patch) {
  ...
  const updated = {
    client_name:
      patch.client_name !== undefined && patch.client_name !== null && (patch.client_name || "").trim() !== ""
        ? patch.client_name.trim()
        : existing.client_name,
    task_name:
      patch.task_name !== undefined && patch.task_name !== null && (patch.task_name || "").trim() !== ""
    ...
    assigned_date:
      patch.assigned_date !== undefined && patch.assigned_date !== null && (patch.assigned_date || "").trim() !== ""
        ? patch.assigned_date
        : existing.assigned_date,
    deadline_date:
      patch.deadline_date !== undefined && patch.deadline_date !== null && (patch.deadline_date || "").trim() !== ""
        ? patch.deadline_date

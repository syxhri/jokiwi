...
export async function updateCategory(userId, id, patch) {
  ...
  await pool.query(
    ...
    [updated.name, updated.description, updated.notes, Number(id), Number(userId)],
  );
...
export async function deleteCategory(userId, id) {
  ...
  await pool.query('DELETE FROM categories WHERE id = $1 AND user_id = $2', [Number(id), Number(userId)]);
  return true;
}
...
export async function getAllOrdersForUser(userId) {
  ...
  const orders = res.rows.map(row => ({
    ...
    assigned_date: row.assigned_date ? row.assigned_date.toISOString().slice(0, 10) : null,
    deadline_date: row.deadline_date ? row.deadline_date.toISOString().slice(0, 10) : null,
  }));
 ...
 * @returns {Promise<Array>}
 */
export async function filterOrders({ userId, search, isDone, isPaid, categoryId, sortBy, sortDir }) {
...
export async function updateOrder(userId, id, patch) {
  ...
  const updated = {
    client_name: patch.client_name !== undefined ? patch.client_name.trim() : existing.client_name,
    task_name: patch.task_name !== undefined ? patch.task_name.trim() : existing.task_name,
    categoryId,
    category_name,
    price: patch.price !== undefined ? Number(patch.price) || 0 : existing.price,
    is_done: patch.is_done !== undefined ? Boolean(patch.is_done) : existing.is_done,
    is_paid: patch.is_paid !== undefined ? Boolean(patch.is_paid) : existing.is_paid,
    notes: patch.notes !== undefined ? patch.notes : existing.notes,
    assigned_date: patch.assigned_date !== undefined ? patch.assigned_date : existing.assigned_date,
    deadline_date: patch.deadline_date !== undefined ? patch.deadline_date : existing.deadline_date,
  };
  await pool.query(

...
function PaymentDialog({ order }) {
  ...
  const qrisWithAmount = order.jokiQrisPayload && order.price ? buildQrisWithAmount(order.jokiQrisPayload, order.price) : null;
  const waMsg = "Halo, saya ingin konfirmasi pembayaran pesanan *" + order.orderCode + "* - " + order.taskName + ".\n\nMohon dikonfirmasi ya, terima kasih!";
  const waLink = order.jokiWhatsapp ? "https://wa.me/" + order.jokiWhatsapp.replace(/[^0-9]/g, "").replace(/^0/, "62") + "?text=" + encodeURIComponent(waMsg) : null;
  ...
  return (
    <div className="rounded-2xl border-2 border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-900/20 p-5 space-y-4">
      <h2 className="font-semibold text-amber-900 dark:text-amber-200">Selesaikan Pembayaran</h2>
      <div className="space-y-1 text-sm text-amber-800 dark:text-amber-300">
        <p><span className="font-semibold">Total Bayar: </span>Rp {Number(order.price || 0).toLocaleString("id-ID")}</p>
        {order.estimatedHours && <p><span className="font-semibold">Estimasi: </span>{order.estimatedHours} jam</p>}
      </div>
      <div className="rounded-xl bg-white dark:bg-slate-900 border border-amber-200 dark:border-amber-800 p-4 space-y-2">
        <p className="text-sm font-semibold text-gray-800 dark:text-gray-100">Cara Bayar &amp; Kirim Bukti:</p>
        <ol className="text-xs text-gray-600 dark:text-gray-400 space-y-1 list-decimal list-inside">
          <li>Scan QRIS di bawah atau transfer manual ke penjoki</li>
          <li>Kirim bukti pembayaran ke WhatsApp penjoki:{" "}
            {waLink ? <a href={waLink} target="_blank" rel="noopener noreferrer" className="font-semibold text-primary-600 underline">Chat penjoki</a>
      ...
      {qrisWithAmount && (
        <div className="space-y-2">
          <p className="text-xs font-semibold text-amber-800 dark:text-amber-300">QR Pembayaran (sudah include nominal):</p>
...
function PushBanner({ orderCode, onSubscribed }) {
  ...
  useEffect(() => {
    ...
    if (Notification.permission === "granted") { autoSubscribe(); }
...
export default function TrackOrderPage() {
  ...
  const fetchOrder = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) { const data = await res.json().catch(() => ({})); setError(data.error || "Order tidak ditemukan"); return; }
      const data = await res.json(); setOrder(data);
    } catch { setError("Gagal mengambil data pesanan"); } finally { setLoading(false); }
  }, [orderCode]);
  ...
  async function handleDownload() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setDownloadError(data.code === "FILE_NOT_FOUND" ? "File sudah tidak tersedia (dihapus otomatis). Hubungi penjoki untuk upload ulang." : data.error || "Gagal mengunduh file");
        return;
      }
      const blob = await res.blob();
      const contentDisposition = res.headers.get("content-disposition") || "";
      const match = contentDisposition.match(/filename\*?=(?:UTF-8'')?["']?([^"';]+)/i);
      const filename = match?.[1] ? decodeURIComponent(match[1]) : order?.taskName?.replace(/[^a-zA-Z0-9]/g, "_") || "hasil";
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a"); a.href = url; a.download = filename;
      document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
      setTimeout(fetchOrder, 1000);
    } catch { setDownloadError("Terjadi kesalahan saat mengunduh"); } finally { setDownloading(false); }
  }

  const statusCfg = order ? (STATUS_CONFIG[order.status] || STATUS_CONFIG.manual) : null;
  const showPaymentDialog = order && (order.status === "accepted" || order.status === "done") && !order.isPaid;
      ...
      <div className="mx-auto max-w-lg space-y-5">
        ...
        <div className="rounded-2xl bg-white dark:bg-slate-900 shadow-lg border border-gray-100 dark:border-slate-800 p-5 space-y-4">
          ...
          <div className="border-t border-gray-100 dark:border-slate-800 pt-3 space-y-1 text-sm">
            <div className="flex justify-between"><span className="text-gray-500">Tugas</span><span className="font-medium text-gray-900 dark:text-gray-100 text-right max-w-[60%]">{order.taskName}</span></div>
            {order.categoryName && <div className="flex justify-between"><span className="text-gray-500">Kategori</span><span className="font-medium text-gray-900 dark:text-gray-100">{order.categoryName}</span></div>}
            {order.deadlineDate && <div className="flex justify-between"><span className="text-gray-500">Deadline</span><span className="font-medium text-gray-900 dark:text-gray-100">{order.deadlineDate}</span></div>}
            {(order.status === "accepted" || order.status === "done") && order.price && <div className="flex justify-between"><span className="text-gray-500">Harga</span><span className="font-semibold text-gray-900 dark:text-gray-100">Rp {Number(order.price).toLocaleString("id-ID")}</span></div>}
            {(order.status === "accepted" || order.status === "done") && order.estimatedHours && <div className="flex justify-between"><span className="text-gray-500">Estimasi</span><span className="font-medium text-gray-900 dark:text-gray-100">{order.estimatedHours} jam</span></div>}
            <div className="flex justify-between"><span className="text-gray-500">Status Bayar</span><span className={"font-semibold " + (order.isPaid ? "text-green-600" : "text-amber-600")}>{order.isPaid ? "Lunas" : "Belum Dibayar"}</span></div>
          </div>
        </div>

        {order.status !== "rejected" && <PushBanner orderCode={orderCode} onSubscribed={fetchOrder} />}
        ...
        {order.status === "done" && (
          <div className="rounded-2xl bg-white dark:bg-slate-900 shadow border border-gray-100 dark:border-slate-800 p-5 space-y-3">
            <h2 className="font-semibold text-gray-900 dark:text-gray-50">Hasil Pengerjaan</h2>
            {!order.isPaid ? (
              <p className="text-sm text-amber-700 dark:text-amber-400">Selesaikan pembayaran terlebih dahulu untuk mengakses Hasil Pengerjaan.</p>
            ) : order.hasExternalLink ? (
              <div className="space-y-3">
                <p className="text-sm text-gray-600 dark:text-gray-300">Hasil Pengerjaan tersedia via link external (Google Drive / Cloud Storage):</p>
                {order.externalLink ? <a href={order.externalLink} target="_blank" rel="noopener noreferrer" className="btn btn-primary w-full inline-flex items-center justify-center gap-2">Buka Link Hasil (Google Drive / Mega)</a> : <p className="text-sm text-amber-600">Link tidak tersedia.</p>}
              </div>
            ) : !order.hasFile ? (
              ...
              <>
                {downloadError && <div className="rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 p-2 text-xs text-red-700 dark:text-red-300">{downloadError}</div>}
                <button onClick={handleDownload} disabled={downloading} className="btn btn-primary w-full">{downloading ? "Mengunduh..." : "Download File Hasil"}</button>
                <p className="text-[11px] text-gray-400 text-center">File akan otomatis dihapus dari server 15 menit setelah pertama kali didownload demi keamanan.</p>
              </>
            )}
          </div>
        )}
        ...
        <p className="text-center text-xs text-gray-400">Simpan link halaman ini untuk melihat status orderanmu nanti.</p>
      </div>
    </div>
  );
}
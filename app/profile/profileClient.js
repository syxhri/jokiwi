...
export default function ProfileClient({ user }) {
  ...
  async function onFileChange(e) {
    ...
    try {
      ...
      if (!payload) {
        setStatus('Gagal membaca QR dari gambar. Coba pakai gambar yang lebih jelas.');
        return;
      }
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-6">
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="text-xs text-gray-500">{hasQris ? 'QRIS tersimpan' : 'Belum ada QRIS'}</span>
      </div>

      <div className="mt-4 rounded-2xl bg-white p-4 shadow-sm space-y-4">
        <div className="space-y-1">
          <p className="text-sm text-gray-700"><span className="font-semibold">Username:</span> {user.username}</p>
          <p className="text-sm text-gray-700"><span className="font-semibold">Nama:</span> {user.name || '-'}</p>
        </div>

        <div className="h-px bg-gray-100" />

        <div className="space-y-2">
          <p className="text-sm font-semibold text-gray-800">Upload QRIS (statis)</p>
          <p className="text-xs text-gray-500">
            Upload gambar QRIS kamu. Sistem akan membaca QR menjadi teks, lalu teks itu dipakai untuk generate QRIS dinamis per order.
          </p>
          ...
          {payloadPreview && (
            <details className="text-xs text-gray-500">
              <summary className="cursor-pointer">Lihat payload yang terbaca</summary>
